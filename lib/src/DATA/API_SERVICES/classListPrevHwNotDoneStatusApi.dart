...
class ClassListPrevHwNotDoneStatusApi {
  Future<List<ClassListPrevHwNotDoneStatusModel>> getClassList(
      Map<String, String?> classList) async {
    print(" classList before API: $classList");
    try {
      http.Response response = await http.post(
        Uri.parse(ApiEndpoints.getClassListPrevHwNotDoneStatusAdmin),
        body: classList,
        headers: headers,
        encoding: encoding,
      );
      ...
      if (response.statusCode == 200) {
        ...
        final classList = (respMap)
            .map((e) => ClassListPrevHwNotDoneStatusModel.fromJson(e))
            .toList();
        return classList;
      }
